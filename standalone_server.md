...
软件授权书dolphindb.lic指定DolphinDB可用的最大内存，用户也可以根据实际情况来配置该值。最大内存限制由配置参数maxMemSize（单位是GB）指定，我们可以在启动DolphinDB时指定该参数：
...
> 3. 在linux环境部署时，volumes文件夹，建议不要指定用NAS挂在服务器路径的远程磁盘。这样做数据库的性能会很差。
如果非要这样做，如果您的分区挂载用的是NFS协议，则该数据库进程**必须**以root身份启动(普通用户启动的数据库进程无权限在NAS读写磁盘，sudo用户启动，则会造成文件夹权限混乱)。
...
6. 重新启动单节点，GUI连接该节点，执行以下命令查看版本信息，检查升级是否成功：
   ...
   version()
   ```
7. 也可以使用 clusterDemo/upgrade.sh 来升级
